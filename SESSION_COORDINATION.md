...
- **Backup tables go in the `backup` schema, not wherever the source table lives** (added 2026-09-13): 7 backup tables scattered across `analytics`/`raw` — including one from 2026-05-26 nobody remembered — got moved to `backup.*`, and 2 of them were anon/authenticated-readable in their old location (one had RLS disabled entirely). Before this, the convention was `[table]_backup_YYYYMMDD` left in place next to the live table — that's how these got forgotten and left exposed. Going forward: create backups directly as `backup.[table]_YYYYMMDD` (e.g. `CREATE TABLE backup.raw_sod_2023_backup AS SELECT * FROM raw.raw_sod WHERE ...`), not in the source schema. `backup` isn't in Supabase's exposed-schemas list, so nothing there is reachable via PostgREST regardless of grants — but still explicitly `REVOKE ALL ... FROM anon, authenticated` and `ENABLE ROW LEVEL SECURITY` on anything landing there, belt-and-suspenders.
