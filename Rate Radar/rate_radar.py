...
# SUPABASE_URL / SUPABASE_KEY moved to shared.py (imported above)
try:
    import anthropic
...
from call_reports import (
    load_call_reports, CALL_REPORTS_DIR,
    _parse_quarter_folder, _find_schedule_file,
)

# ── Rate extraction (regex parsing of scraped page text) ─────────────────────
from extraction import (
    tag_rate_context, check_rate_staleness, detect_rebrand, extract_domain,
    parse_json_object, _clean_cd_ladder, extract_rates_from_json,
    extract_rates, _parse_term_months, JSON_RATE_KEY_PAT,
)

# ── Shared cross-cutting state & config (crawl_state, Supabase env, URL lists) ─
from shared import (
    SUPABASE_URL, SUPABASE_KEY, crawl_state, _estimate_cost_usd,
    RATE_PATHS, BANK_EXTRA_URLS, FIELDS,
    CIRCUIT_BREAKER_THRESHOLD, CIRCUIT_BREAKER_PROBE_EVERY,
)

# ── Storage: Supabase push + local export (see storage.py) ───────────────────
from storage import (
    build_supabase_rows, fetch_last_known_good, push_to_supabase,
    push_rate_observations, load_bank_url_config, _bank_health, _circuit_open,
    _content_hash, _extraction_cache_get, _extraction_cache_put,
    update_bank_registry, push_run_summary, auto_save,
)

# ── Scrapers: Playwright crawl orchestration (see scrapers.py) ───────────────
# All crawling/scraping logic (crawl_bank, ai_agent_crawl, chat_crawl,
# run_crawler, and their helpers) moved to scrapers.py — only the thread
# launcher is called directly from a Flask route.
from scrapers import start_crawl_thread



# ── Export / auto-save ────────────────────────────────────────────────────────

# EXPORTS/FIELDS/SB_NUMERIC_FIELDS/CIRCUIT_BREAKER_* moved to shared.py; build_supabase_rows through auto_save moved to storage.py (both imported above)
