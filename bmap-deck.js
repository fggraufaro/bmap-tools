/**
 * bmap-deck.js — BMAP Snapshot Deck Builder v2
 * FIXES:
 *  - branchCount / branchList rename (was colliding with branches array)
 *  - Chart colors improved with proper hex, better legend, pointStyle
 *  - pie chart border white so slices are distinct
 *  - bar chart labels added
 *  - JSZip loaded eagerly to avoid async fail on export
 */
...
// ── Logo — fetched from CDN to keep file size manageable ───────
const VLOGO_URL = 'https://fggraufaro.github.io/bmap-tools/BMAP.png';
let VLOGO = null; // will be populated as base64 on first export

async function ensureLogo() {
  if (VLOGO) return;
  try {
    const resp = await fetch(VLOGO_URL);
    const blob = await resp.blob();
    VLOGO = await new Promise((res, rej) => {
      const r = new FileReader();
      r.onload = () => res(r.result.split(',')[1]);
      r.onerror = rej;
      r.readAsDataURL(blob);
    });
    VLOGO = 'image/png;base64,' + VLOGO;
  } catch(e) {
...
function vChart(type, labels, datasets, opts={}) {
  return new Promise(resolve => {
    ...
    // Nice legend config shared across chart types
    const legendCfg = {
    ...
    const ch = new Chart(c.getContext('2d'), {
      ...
      options: {
        animation: false,
        responsive: false,
        layout: { padding: { top:12, bottom: opts.pb||16, left:12, right:12 } },
        plugins: {
          legend: legendCfg,
          title: { display: false }
        },
        scales: (type==='doughnut'||type==='pie') ? {} : {
          x: {
            ticks: {color:'#555', font:{size:13, family:'Calibri', weight:'600'}},
            grid: {display:false},
...
function addChrome(slide, pageNum, label) {
  ...
  if (VLOGO) slide.addImage({data:VLOGO, x:0.42,y:5.04,w:1.55,h:0.36});
  if (label) {
...
// ══════════════════════════════════════════════════════════════
// SLIDE 1 — COVER
// NOTE: uses branchCount (string), NOT branchList (array)
// ══════════════════════════════════════════════════════════════
function buildCover(pres, d) {
  ...
  if (VLOGO) s.addImage({data:VLOGO,x:1.2,y:0.28,w:2.45,h:0.54});
  ...
  // ── 4 KPI tiles — use branchCount (string), not branchList ──
  const kpis=[
    {v: d.branchCount,  l:'BRANCHES'},
  ...
  if (VLOGO) s.addImage({data:VLOGO,x:0.42,y:5.04,w:1.55,h:0.36});
...
async function buildNetwork(pres, d) {
  ...
  // Pie chart — zone distribution with proper colors + white borders
...
// Uses d.branchList (array), not d.branchCount
// ══════════════════════════════════════════════════════════════
async function buildBranches(pres, d) {
  ...
  d.branchList.slice(0,5).forEach((b,i)=>{
...
function buildFinancial(pres, d) {
  ...
  const cols = [
    {x:6.22,w:1.12,label:'',align:'left'},
    {x:7.38,w:1.26,label:'VALUE'},
    {x:8.68,w:1.0, label:'BENCHMARK'}
  ];
  cols.forEach(col=>{
...
async function buildGap(pres, d) {
  ...
  // Bar chart — bank vs peer with named labels and better colors
  const bankColor  = d.gapNeg ? '#A32D2D' : '#1D9E75';
  const peerColor  = '#4A6A8A';
  const gapImg = await vChart('bar',
    ['This Bank','Peer Avg'],
    [{
      label: 'Deposit YoY %',
      data: [parseFloat(d.bankYoY), parseFloat(d.peerYoY)],
      backgroundColor: [bankColor, peerColor],
      borderColor:     [bankColor, peerColor],
      borderWidth: 0,
      borderRadius: 4,
      borderSkipped: false
    }],
...
// ── Fix notesMasterIdLst XML ordering (PptxGenJS quirk) ────────
async function fixPptxPresXml(blob) {
  // JSZip should already be on page; if not, load it
  ...
  if (zip.files['ppt/presentation.xml']) {
    ...
    // Remove notesMasterIdLst entirely — no notes slides in this deck
    xml = xml.replace(/<p:notesMasterIdLst>[\s\S]*?<\/p:notesMasterIdLst>/g, '');
...
{"slides":[
  {"id":"network","headline":"strong claim max 9 words","spoken":"2 sentences Tom says walking in. Specific numbers.","bullets":["data point with number","competitive insight","risk or opportunity"],"close":"one punchy closing line"},
  ...
  {"id":"nextsteps","headline":"...","spoken":"...","bullets":[...],"close":"..."}
    ...
    // ── Build DATA ─────────────────────────────────────────────
    // NOTE: branchCount = string for cover KPI tile
    //       branchList  = array for branch slide cards
    if (btn) btn.textContent='Step 3/4 — Drawing charts...';
    const DATA = {
      bankName,
      date: new Date().toLocaleDateString('en-US',{month:'long',year:'numeric'}),
      branchCount: String(rows.length),   // ← STRING for cover tile
      ...
      branchList: topBr.slice(0,8).map(b=>({   // ← ARRAY for branch slide
    ...
    await buildBranches(pres, DATA);
    buildFinancial(pres, DATA);
    await buildGap(pres, DATA);
    buildNextSteps(pres, DATA);

    // Generate blob, fix XML, download
    const rawBlob = await pres.write('blob');
    const fixedBlob = await fixPptxPresXml(rawBlob);
